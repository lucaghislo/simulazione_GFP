...
for row = 0:5
    for mod = 0:5
        ...
        if(length(data_landau) > 0)
            [vpp, sig, mv, bound] = histfitlandau(data_landau, 3, 0, 2047);
...
ylabel("\textbf{[ADU]}")
ylim([0 1000])
legend("Landau distribution MPV, $\mu = " + string(round(nanmean(vpp), 2)) + "$ ADU, $\sigma = " + string(round(nanstd(vpp), 2)) + "$ ADU", "Landau distribution $\sigma$ [ADU]")
