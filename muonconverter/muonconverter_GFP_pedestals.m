...
% OUTPUT
%  muon_allch_out_ped: muon data output after conversion (in keV, single pedestal subtraction)
...
function [muon_allch_out_ped, muon_allch_out_inj, muon_allch_out_rem, landau_MPV] = muonconverter_GFP_pedestals(row, module, data_in_path, folder_out_path, pt_in, fdt_data, pedestal_data, ch_start, ch_finish, bin_width, max_kev)
    ...
    if ch_count > 0
        ...
        spline_allchs_pt_ped = nan(length(range), (ch_finish - ch_start) + 1);
        ch_count = 0;
        
        % Calcolo spline per definizione lookup table sui canali di interesse
        % per la conversione
        fdt_data_allch_noped_ped = fdt_data_allch;
        disp("Calcolo spline");
        for ch = ch_values
            fdt_data_ch_raw = fdt_data_allch(:, ch_count + 1);
            fdt_data_ch_ped = fdt_data_ch_raw - pedestal_data_allch(ch_count+1);
            fdt_data_ch_inj = fdt_data_ch_raw - fdt_CAL10_allch(ch+1);
            fdt_data_allch_noped_ped(:, ch_count + 1) = fdt_data_ch_ped;
            fdt_data_allch_noped_inj(:, ch_count + 1) = fdt_data_ch_inj;
            spline_allchs_pt_ped(:, ch_count + 1) = interp1(dac_values, fdt_data_ch_ped, range, 'spline');
            spline_allchs_pt_inj(:, ch_count + 1) = interp1(dac_values, fdt_data_ch_inj, range, 'spline');
            disp("Canale: " + string(ch));
        
            [val_ped, idx_ped] = unique(spline_allchs_pt_ped(:, ch_count + 1));
            [val_inj, idx_inj] = unique(spline_allchs_pt_inj(:, ch_count + 1));
        
            for i = 1:length(spline_allchs_pt_ped(:, ch_count + 1))
                if isempty(find(idx_ped==i))
                    spline_allchs_pt_ped(i, ch_count + 1) = spline_allchs_pt_ped(i, ch_count + 1) + step_DACinj/2;
                end
            end

            for i = 1:length(spline_allchs_pt_inj(:, ch_count + 1))
                if isempty(find(idx_inj==i))
                    spline_allchs_pt_inj(i, ch_count + 1) = spline_allchs_pt_inj(i, ch_count + 1) + step_DACinj/2;
                end
            end
        ...
        for ch = ch_values
            ...
            row_counter = out_row_ch_counter;
            for i = [1:length(events_kev_ped)]
                muon_allch_ped(row_counter, 1) = ch_sel;
                muon_allch_ped(row_counter, 2) = events_kev_ped(i);
                row_counter = row_counter + 1;
            end

            row_counter = out_row_ch_counter;
            for i = [1:length(events_kev_inj)]
                muon_allch_inj(row_counter, 1) = ch_sel;
        ...
        muon_allch_ped = muon_allch_ped(:, 2);
        muon_allch_out_ped = muon_allch_ped;
        ...
        hold on
        for ch = ch_values
            plot(dac_values.*0.841, fdt_data_allch_noped_ped(:, ch_count + 1).*0.841);
            ch_count = ch_count + 1;
        end
        hold off
        ...
        exportgraphics(gcf, folder_out_path + "/transfer_function_pt" + string(pt) + "_ch" + string(ch_start) + "-" + string(ch_finish) +"_no-pedestal_ped.pdf",'ContentType','vector');
        disp("SAVED: transfer_function_pt" + string(pt) + "_ch" + string(ch_start) + "-" + string(ch_finish) +"_no-pedestal_ped.pdf")
        pedestal_diff = abs(fdt_CAL10_allch - pedestal_data_allch);

        % Plot funzione di trasferimento successivamente alla sottrazione del
        % piedistallo DA INIEZIONE
        ch_count = 0;
        f = figure("Visible", "off");
        hold on
        for ch = ch_values
            plot(dac_values.*0.841, fdt_data_allch_noped_inj(:, ch_count + 1).*0.841);
            ch_count = ch_count + 1;
        end
        hold off
        ...
        yticks([0:200:2000])
        set(gcf, 'Color', 'w');
        title("\textbf{Transfer function for channels " + string(ch_start) + " - " + string(ch_finish) + " at \boldmath$\tau_{" + string(pt) + "}$ with pedestal subtracted (no injection)}")
        ...
        exportgraphics(gcf, folder_out_path + "/transfer_function_pt" + string(pt) + "_ch" + string(ch_start) + "-" + string(ch_finish) +"_no-pedestal_inj.pdf",'ContentType','vector');
        disp("SAVED: transfer_function_pt" + string(pt) + "_ch" + string(ch_start) + "-" + string(ch_finish) +"_no-pedestal_inj.pdf")
        pedestal_diff = abs(fdt_CAL10_allch - pedestal_data_allch);
