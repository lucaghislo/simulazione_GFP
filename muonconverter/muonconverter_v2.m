...
% 3) muonconverter(data_in_path, plot_out_path, pt_in, fdt_data, pedestal_data, ch_start, ch_finish, landau_flag, fig_on, bin_width, max_kev)
% full custom parameters

function [muon_allch_out] = muonconverter_v2(data_in_path, folder_out_path, pt_in, fdt_data, pedestal_data, ch_start, ch_finish, landau_flag, fig_on, bin_width, max_kev)
...
muon_allch_out = sortrows(muon_allch_out);
max_length = max(length())
muon_allch = reshape(muon_allch', [], 1);

% Figure on/off depending on user decision
if fig_on
    fig_on_flag = "on";
else
    fig_on_flag = "off";
...
% Landau fit yes/no depending on user decision
% TODO settare dinamicamente taglio piedistallo ed interpolazione
% gaussiana
if landau_flag
...
% (old) Output di un singolo vettore con tutti gli eventi in keV senza
% distinzione del canale
% muon_allch_out = muon_allch(~isnan(muon_allch));
...
exportgraphics(gcf, folder_out_path, 'ContentType', 'vector');
