function [vpp,sig,mv,bound]=histfitlandau(val,passo,inizio,fine,mult)
...
yfit=fitres(xfit);
line(xfit,yfit,'LineWidth',1.5,'LineStyle','-','Color', [colors(2, 1), colors(2, 2), colors(2, 3)]);
picco=max(yfit);
...
testo={['MPV: ', num2str(round(mpv*mult, 2)), ' ADU'];['$\sigma$ = ', num2str(round(fitres.sigma*mult, 2)), ' ADU'];['Entries: ' num2str(length(val))]};
annotation('textbox',[.67 .805 .1 .1],'FitHeightToText','on','String',testo,'Fontsize',18, 'BackgroundColor', 'white');
if nargout
    vpp=mpv;
    sig=fitres.sigma;
    mv=mean(val);
